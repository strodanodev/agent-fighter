-- Agent Fighter — AGENT ARCADE (ranked gauntlet). Run AFTER 0007.
--
-- The old single-match "solo" mode is consolidated into AGENT ARCADE: one
-- ARCADE_FEE (1) credit escrows with BATTLE 1 of a run and is CONSUMED by
-- playing; every battle settles here (mode = 'arcade') on the same ranked
-- XP/W-L ladder as solo. Credits move only via the NEW explicit `_payout`
-- argument, which the match server computes from the run position:
--   +1 at the ceil(N/3) and ceil(2N/3) milestone battles,
--   +5 for clearing all N agents.
-- (Mirrored in packages/server/src/persist.ts settleSide — keep in sync.)
--
-- record_match gains `_payout integer default 0` at the END of the argument
-- list. The already-deployed server calls by named args WITHOUT _payout, so
-- the default keeps it fully backward-compatible — but the old signature must
-- be DROPPED first or PostgREST sees two overloads and 300s every call.

drop function if exists record_match(text, text, integer, text, text, text, text,
  boolean, boolean, text, text, text, text, smallint, text, smallint, smallint,
...
begin
  ...
  foreach s in array array[0::smallint, 1::smallint] loop
    ...
    if undecided then
      ...
      -- AGENT ARCADE battle: entry fee consumed by playing (a win does NOT
      -- return it); credits move only via the server-computed milestone
      -- payout. A draw refunds the fee (battle 1 only carries one).
