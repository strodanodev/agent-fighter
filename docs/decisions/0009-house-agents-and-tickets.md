...
| Season length | ~~Placeholder 21 days~~ → **SEASON 0 · OPEN BETA, clock FROZEN at 0** (owner decision 2026-07-29, migration 0029): the current period is the beta, `current_season()` returns 0 unconditionally, and existing beta stamps/tickets were renumbered 1→0 with ratings preserved. **Season 1's start date is an explicit owner decision, made after 2026-08-17** — a future migration restores epoch arithmetic anchored at the chosen date; the lazy rollover then re-bases everyone on their first season-1 match. Tickets, season score, and gate rotation still share that ONE boundary | The 21-day placeholder would have auto-rolled on Aug 17 and wiped every beta rating on a boundary nobody decided. A season starts when the owner says so |
