import { Btn, held, pressed } from './input.js';
import type { InputFrame } from './input.js';
import { FP, clamp, fp, fpToPx } from './fp.js';
import { STAGE } from './data.js';
import type { CharacterDef, MoveStep, Rect } from './data.js';
...
const worldRect = (f: FighterState, rc: Rect): WorldRect => {
  const rx = fp(rc.x);
  const rw = fp(rc.w);
...
const attackStep = (f: FighterState, def: CharacterDef): MoveStep | null => {
  if (f.action !== Action.Attack) return null;
  let acc = 0;
  for (const step of def.attack.steps) {
    acc += step.frames;
    if (f.actionFrame < acc) return step;
  }
  return null;
};

const totalAttackFrames = (def: CharacterDef): number =>
  def.attack.steps.reduce((n, s) => n + s.frames, 0);

const currentHurtbox = (f: FighterState, def: CharacterDef): Rect => {
...
const updateFighter = (f: FighterState, input: InputFrame, def: CharacterDef): void => {
  ...
  switch (f.action) {
    ...
    case Action.Hitstun:
      f.hitstunLeft--;
      if (f.hitstunLeft <= 0 && grounded(f)) setAction(f, Action.Idle);
      break;

    case Action.Attack: {
      f.actionFrame++;
      if (f.actionFrame >= totalAttackFrames(def)) setAction(f, Action.Idle);
      break;
    }
    ...
    case Action.Idle:
    case Action.Walk: {
      if (pressed(input, prev, Btn.Attack)) {
        f.velX = 0;
        f.attackHasHit = 0;
        setAction(f, Action.Attack);
        break;
      }
      if (held(input, Btn.Up)) {
        f.velY = def.jumpVelY;
        f.velX = held(input, Btn.Left) ? -def.walkSpeed : held(input, Btn.Right) ? def.walkSpeed : 0;
        setAction(f, Action.Jump);
        break;
      }
      const dir = (held(input, Btn.Right) ? 1 : 0) - (held(input, Btn.Left) ? 1 : 0);
      f.velX = dir * def.walkSpeed;
      const next = dir !== 0 ? Action.Walk : Action.Idle;
  ...
  if (!grounded(f) || f.velY < 0) f.velY += def.gravity;
  f.x += f.velX;
  f.y += f.velY;
  if (f.y >= FLOOR) {
    f.y = FLOOR;
    f.velY = 0;
    if (f.action === Action.Hitstun && f.hitstunLeft <= 0) setAction(f, Action.Idle);
  }
  f.x = clamp(f.x, WALL_L, WALL_R);

  // Hitstun ground friction.
  if (f.action === Action.Hitstun && grounded(f)) {
...
const applyHit = (victim: FighterState, attacker: FighterState, dmg: number, stun: number, push: number): void => {
  victim.health = Math.max(0, victim.health - dmg);
  victim.hitstunLeft = stun;
  victim.velX = attacker.facing * push;
  victim.velY = 0;
  setAction(victim, victim.health <= 0 ? Action.KO : Action.Hitstun);
};
...
export const step = (s: GameState, inputs: [InputFrame, InputFrame]): void => {
  ...
  for (const [me, other] of [[f0, f1], [f1, f0]] as const) {
    if ((me.action === Action.Idle || me.action === Action.Walk) && grounded(me)) {
  ...
  const hit0 = // does f0's attack connect this tick?
    step0?.hitbox && !f0.attackHasHit && f1.action !== Action.KO &&
    overlaps(worldRect(f0, step0.hitbox.rect), worldRect(f1, currentHurtbox(f1, c1)))
      ? step0.hitbox : null;
  const hit1 =
    step1?.hitbox && !f1.attackHasHit && f0.action !== Action.KO &&
