...
import type { CharacterDef } from './data.js';
...
export interface FighterState {
  ...
  health: number;
  action: Action;
  actionFrame: number; // frames elapsed in current action
  hitstunLeft: number;
  attackHasHit: number; // 0/1 — current attack already connected
  prevInput: number;
}
...
export interface GameState {
  ...
  rngSeed: number;
  phase: Phase;
  winner: number; // -1 none, 0/1 player index, 2 draw
  timerTicks: number;
  hitstopLeft: number; // global freeze frames (both fighters)
  fighters: [FighterState, FighterState];
}

/** M0: both players use the rectangle archetype. Later: per-player bundles. */
export const characters: [CharacterDef, CharacterDef] = [RECT_FIGHTER, RECT_FIGHTER];

const spawnFighter = (x: number, facing: 1 | -1, def: CharacterDef): FighterState => ({
  x: fp(x),
  y: fp(STAGE.floorYPx),
  velX: 0,
  velY: 0,
